...
| `auto_hide`   | `[call]`   | Automatically hide the regular menu while any listed condition is active. Supported values are `call` and `casting`.                                                                                                                                         |
...
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `call`         | The `call` menu button starts the call flow for the active camera. It only appears when the active camera has [`call.stream`](./cameras/call-mode.md) configured, the button itself is enabled, no call is already active, and the current live view is not using a substream override.                                                                                                                                                                                                                |
...
| `microphone`   | The `microphone` button allows usage of 2-way audio in certain non-live contexts. It is not shown in the live view; live 2-way audio starts with the `call` button and uses the in-call microphone control instead. See [Using 2-way audio](../usage/2-way-audio.md).                                                                                                                                                                                                                                 |
